...
describe("ClearanceResponseSchema", () => {
    ...
    it("should default policyVersionId and policyContentHash to null", () => {
        const resp = ClearanceResponseSchema.parse({ approved: true });
        expect(resp.policyVersionId).toBeNull();
        expect(resp.policyContentHash).toBeNull();
    });
    ...
    it("should parse policy version metadata from wire payload", () => {
        ...
        const resp = ClearanceResponseSchema.parse(wire);
        expect(resp.policyVersionId).toBe("3b6b2e1d-9d7d-4a32-9f6e-54a4b0ee8e11");
        expect(resp.policyContentHash).toBe("sha256:abcd1234");
    });
});
